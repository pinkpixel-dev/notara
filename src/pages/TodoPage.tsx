...
import { Edit3, Trash2, ListChecks, Plus, Calendar, CheckCircle2, Clock } from 'lucide-react';
...
import { isDesktopRemindersSupported } from '@/lib/reminders/client';
...
const TodoPage: React.FC = () => {
  ...
  const [newItemReminderEnabled, setNewItemReminderEnabled] = useState(false);
  ...
  const isDesktop = useMemo(() => isDesktopRemindersSupported(), []);
  const focusTarget = useMemo(
    () => ({ kind: 'todo-list' as const, listId: selectedListId }),
    [selectedListId]
  );
  ...
  const openEditItemDialog = (item: TodoItem) => {
    ...
    setNewItemReminderEnabled(Boolean(item.reminderEnabled));
    setIsEditingItem(true);
  };

  const handleEditItem = () => {
    if (!selectedListId || !editingItemId) return;
    updateTodoItem(selectedListId, editingItemId, {
      content: newItemContent,
      time: newItemTime,
      reminderEnabled: newItemReminderEnabled,
    });
  ...
  const toggleExpandItem = (itemId: string) => {
    setExpandedItemIds((prev) =>
      prev.includes(itemId) ? prev.filter((id2) => id2 !== itemId) : [...prev, itemId]
    );
  };
  ...
  return (
    <AppLayout>
              ...
              {selectedList ? (
                      ...
                      <div className="space-y-3">
                        {sortedItems.map((item) => (
                          <TodoItemRow
                            ...
                            subInput={subInputs[item.id] || ''}
                            onToggleExpand={toggleExpandItem}
                            onToggleCheck={(checked) =>
                              updateTodoItem(selectedList.id, item.id, { checked })
                            }
                            onToggleReminder={() =>
                              toggleItemReminder(selectedList.id, item.id)
                            }
                            onEditItem={(it) => {
                              openEditItemDialog(it);
                              setIsManagingItems(true);
                            }}
                            onDeleteItem={handleDeleteItem}
                            onSubInputChange={(val) =>
                              setSubInputs((prev) => ({ ...prev, [item.id]: val }))
                            }
                            onAddSubItem={() => handleAddSubItem(selectedList.id, item.id)}
                            onToggleSubCheck={(subId, checked) => {
                              const updatedSubs = (item.subItems || []).map((s) =>
                                s.id === subId ? { ...s, checked } : s
                              );
                              updateTodoItem(selectedList.id, item.id, { subItems: updatedSubs });
                            }}
                            onDeleteSubItem={(subId) =>
                              handleDeleteSubItem(selectedList.id, item.id, subId)
                            }
                          />
                        ))}
                      </div>
                ...
                <div className="h-full flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-muted-foreground mb-6 max-w-md">
                    Get started by creating a to-do list or selecting an existing one from the
                    sidebar.
                  </p>
                  <Button
      ...
      <TodoDialogs
        ...
        newItemReminderEnabled={newItemReminderEnabled}
        setNewItemReminderEnabled={setNewItemReminderEnabled}
        isDesktop={isDesktop}
