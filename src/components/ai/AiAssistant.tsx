...
const readArchiveFromLocalStorage = (): ConversationSnapshot[] => {
  ...
  try {
    const stored = window.localStorage.getItem(LOCAL_AI_ARCHIVE_KEY);
    if (!stored) {
      return [];
    }
    const parsed = JSON.parse(stored) as ConversationSnapshot[];
    return Array.isArray(parsed) ? parsed.slice(0, 20) : [];
  } catch (error) {
    console.warn('Failed to load AI conversation archive:', error);
    return [];
  }
};

const writeArchiveToLocalStorage = (archive: ConversationSnapshot[]): void => {
  if (typeof window === 'undefined') {
    return;
  }
  try {
    window.localStorage.setItem(LOCAL_AI_ARCHIVE_KEY, JSON.stringify(archive.slice(0, 20)));
  } catch (error) {
...
const AiAssistant: React.FC = () => {
  ...
  const [groupByTags, setGroupByTags] = useState<boolean>(false);
    ...
    // New state variables for Vision Board and Constellation View
    const [includeVisionBoards, setIncludeVisionBoards] = useState<boolean>(false);
  ...
  }, [loadAiConversations, storageStatus]);
  ...
  useEffect(() => {
    ...
    writeArchiveToLocalStorage(conversationArchive);

    if (storageStatus === 'ready') {
      void saveAiConversations(conversationArchive).catch((error) => {
        console.error('Failed to persist AI conversations to filesystem', error);
      });
    }
  ...
  const handleLoadConversation = (id: string) => {
    const snapshot = conversationArchive.find(item => item.id === id);
    if (!snapshot) {
      return;
    }

    setMessages(snapshot.messages.map(message => ({ ...message })));
    setInputMessage('');
    setShowImagePrompt(false);
  ...
  const handleDeleteConversation = (id: string) => {
    setConversationArchive(prev => prev.filter(item => item.id !== id));
    toast({
      title: 'Conversation removed',
      description: 'The history panel has been updated.'
    });
  };
  ...
  const handleStartNewConversation = () => {
    ...
    if (wasSaved) {
      toast({
        ...
        description: 'Starting a fresh chat.'
      });
    }
  };
  ...
  const aiMessagesCount = Math.max(totalMessages - userMessagesCount, 0);
  const lastInteraction = messages[messages.length - 1]?.timestamp;
  const lastInteractionLabel = lastInteraction
    ? new Date(lastInteraction).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })
    : null;
  const recentMessages = [...messages].reverse().slice(0, 5);
  const recentSummaries = [...summaryHistory].slice(-3).reverse();
...
`;
  ...
  const handleSummarizeNotes = async () => {
    ...
    try {
      ...
      // Get additional context from Vision Boards if enabled
      let visionBoardContent = '';
      if (includeVisionBoards) {
        visionBoardContent = formatVisionBoardData(selectedVisionBoards);
      }
...
`;
  ...
  const formatVisionBoardData = (visionBoardIds: string[] = []) => {
    ...
    const boardsToFormat = visionBoardIds.length > 0
      ? visionBoards.filter(board => visionBoardIds.includes(board.id))
      : visionBoards;

    if (boardsToFormat.length === 0) return '';

    let formattedData = '## Vision Boards\n\n';
    ...
    boardsToFormat.forEach(board => {
      ...
      if (imageItems.length > 0) {
        ...
        formattedData += `- ${imageItems.length} images are present in this vision board\n`;
        formattedData += '\n';
      }
    });
  ...
  // Analyze connections between notes, vision boards, and constellation view
  const handleAnalyzeConnections = async () => {
    if ((visionBoards.length === 0 && notes.length < 2)) {
      toast({
        title: "Not enough content to analyze",
        description: "You need at least 2 notes or 1 vision board to analyze connections.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      // Format vision board data
      const visionBoardContent = formatVisionBoardData(selectedVisionBoards.length > 0 ? selectedVisionBoards : visionBoards.map(board => board.id));
...
`;
  ...
  const SummaryOptionsDialog = () => {
    ...
    const [localIncludeVisionBoards, setLocalIncludeVisionBoards] = useState<boolean>(includeVisionBoards);
    const [localIncludeConstellationView, setLocalIncludeConstellationView] = useState<boolean>(includeConstellationView);
    const [localSelectedVisionBoards, setLocalSelectedVisionBoards] = useState<string[]>(selectedVisionBoards);
    ...
    useEffect(() => {
      if (showSummaryOptions) {
        ...
        setLocalIncludeVisionBoards(includeVisionBoards);
        setLocalIncludeConstellationView(includeConstellationView);
        setLocalSelectedVisionBoards(selectedVisionBoards);
        ...
        includeVisionBoards, includeConstellationView, selectedVisionBoards, constellationSimilarityThreshold]);
    ...
    const applyChanges = () => {
      ...
      setIncludeVisionBoards(localIncludeVisionBoards);
      setIncludeConstellationView(localIncludeConstellationView);
      setSelectedVisionBoards(localSelectedVisionBoards);
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-4 pt-4 border-t border-border/30">
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="include-visionboards"
                    checked={localIncludeVisionBoards}
                    onCheckedChange={(checked) => setLocalIncludeVisionBoards(!!checked)}
                  />
                  <Label htmlFor="include-visionboards">Include Vision Boards</Label>
                </div>

                {localIncludeVisionBoards && visionBoards.length > 0 && (
                  <div className="pl-6 space-y-2">
                    <Label className="text-xs">Select Vision Boards</Label>
                    <div className="max-h-[150px] overflow-y-auto space-y-1 pr-2">
                      {visionBoards.map(board => (
                        <div key={board.id} className="flex items-center space-x-2">
                          <Checkbox
                            id={`visionboard-${board.id}`}
                            checked={localSelectedVisionBoards.includes(board.id)}
                            onCheckedChange={(checked) => {
                              if (checked) {
                                setLocalSelectedVisionBoards(prev => [...prev, board.id]);
                              } else {
                                setLocalSelectedVisionBoards(prev => prev.filter(id => id !== board.id));
                              }
                            }}
                          />
                          <Label htmlFor={`visionboard-${board.id}`} className="text-sm">
                            {board.name}
                          </Label>
                        </div>
                      ))}
                    ...
                    <div className="flex gap-2 mt-1">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setLocalSelectedVisionBoards(visionBoards.map(board => board.id))}
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setLocalSelectedVisionBoards([])}
                        className="text-xs py-0 h-7"
                      >
                        Clear All
                      </Button>
      ...
      <div className="grid h-full gap-4 xl:grid-cols-[minmax(0,2.25fr)_minmax(300px,1fr)]">
        <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-3xl border border-border/40 bg-card/80 backdrop-blur-xl shadow-xl">
          <div className="flex items-center gap-3 border-b border-border/40 bg-card/40 px-4 py-3 sm:px-6">
            ...
            <div className="flex flex-col">
              <h2 className="text-lg font-semibold sm:text-xl">AI Assistant</h2>
              <span className="hidden text-xs text-muted-foreground sm:block">
                Ask questions, generate ideas, and connect your notes.
              </span>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <span className="hidden rounded-full bg-primary/15 px-3 py-1 font-mono text-xs text-primary md:inline-flex">
                GPT-5
              </span>
              <Button
                variant="ghost"
                ...
                title="Open summarization options"
              >
              ...
              <Button
                ...
                size="icon"
                className="hover:bg-primary/10"
                onClick={() => setShowSummaryHistory(true)}
          ...
          <div
            className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 flex flex-col gap-4"
          ...
          <div className="border-t border-border/40 bg-card/70 backdrop-blur-md px-4 py-3 sm:px-6 sm:py-4">
            ...
            {showImagePrompt && (
              <div className="mb-3 p-3 border border-border/40 rounded-md bg-background/50 animate-slide-down">
            ...
            <div className="flex gap-2">
              <Textarea
                ...
                className="resize-none bg-background/50 border-border/40 focus:border-primary transition-colors"
                onKeyDown={(e) => {
        ...
        <aside className="hidden xl:flex h-full min-h-0 flex-col overflow-hidden rounded-3xl border border-border/40 bg-card/60 backdrop-blur-xl shadow-xl">
          <div className="flex items-center justify-between border-b border-border/30 px-6 py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground/80">Stay aligned with quick stats, saved chats, and summaries.</p>
            </div>
            <span className="rounded-full bg-primary/12 px-3 py-1 text-xs font-mono text-primary">Beta</span>
          </div>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-6 py-5 space-y-5">
            <Card className="glass-panel shadow-none">
              ...
              <CardContent className="space-y-3 text-sm text-muted-foreground">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-primary">{totalMessages}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>You</span>
                  <span className="font-mono">{userMessagesCount}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Assistant</span>
                  <span className="font-mono">{aiMessagesCount}</span>
                </div>
                {lastInteractionLabel && (
                  <div className="flex items-center gap-2 pt-1 text-xs">
                    <Clock className="h-3 w-3 text-primary" />
                    <span>Last reply at {lastInteractionLabel}</span>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="glass-panel shadow-none">
              ...
              <CardContent className="max-h-60 space-y-3 overflow-y-auto pr-1 text-sm text-muted-foreground">
                {recentMessages.length ? (
                  recentMessages.map(message => (
                    ...
                    >
                      <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground/80">
                        <span className="font-medium text-foreground">
                          {message.sender === 'user' ? 'You' : 'Assistant'}
                        </span>
                        <span>{new Date(message.timestamp).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</span>
                      </div>
                      <p className="text-sm text-foreground/80 overflow-hidden text-ellipsis whitespace-nowrap">
                        {message.content.replace(/\s+/g, ' ')}
                      </p>
                    </div>
                  ))
                ) : (
                  <p>Start chatting to populate your timeline.</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="glass-panel shadow-none">
              ...
              <CardContent className="space-y-3 text-sm text-muted-foreground">
                {conversationArchive.length ? (
                  ...
                  <p>Save interesting chats to revisit them later.</p>
                )}
                <Button
                  variant="secondary"
                  size="sm"
              ...
              <CardContent className="space-y-3 text-sm text-muted-foreground">
                {recentSummaries.length ? (
                  recentSummaries.map(summary => {
                    const summaryType = summaryTypes.find(type => type.id === summary.summaryType);
                    return (
                      <div key={summary.id} className="rounded-lg border border-border/30 bg-background/60 p-3">
                        <div className="flex items-center justify-between text-xs text-muted-foreground/80">
                          <span className="font-medium text-foreground">
                            {summaryType?.name ?? 'Summary'}
                          </span>
                          <span>{new Date(summary.timestamp).toLocaleDateString()}</span>
                        </div>
                        <p className="mt-1 text-sm text-foreground/80 overflow-hidden text-ellipsis whitespace-nowrap">
